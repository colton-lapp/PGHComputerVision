...
def return_optimal_panoid(lat, lng):
    ...
    """
    #panoid_list = streetview.panoids(lat, lng) #COLTON: this code seems outdated
    ...
    for panoid in panoid_list:
        try:
            meta_data = streetview.get_panorama_meta(pano_id=panoid.pano_id, api_key=CONFIG.SV_api_key)
            pano_date =  datetime.strptime(meta_data.date + '-01', '%Y-%m-%d').date()
