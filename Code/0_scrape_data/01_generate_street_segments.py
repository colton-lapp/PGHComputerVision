...
# Drop rows with missing geometry COLTON ADDED
street_segments_full = street_segments_full.dropna(subset=['geometry'])
...
for geometry in street_segments_full['geometry']:
    # Extract the coordinates of the first and second points
    coords = np.array(geometry.coords)
    node1_coords = coords[0]
    node2_coords = coords[1]
    
    # Create Point objects for node1 and node2
    node1 = Point(node1_coords)
    node2 = Point(node2_coords)
    
    # Append Point objects to the lists
    node1_list.append(node1)
    node2_list.append(node2)

# Assign the lists to new columns in the DataFrame
street_segments_full['node1'] = node1_list
street_segments_full['node2'] = node2_list
